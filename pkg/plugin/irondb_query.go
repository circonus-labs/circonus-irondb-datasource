...
var segmentTypeNames = []string{ //nolint:gochecknoglobals
	"MetricName",
	"TagCat",
	"TagVal",
	"TagPair",
...
func decodeTag(t string) ([]byte, error) {
	if len(t) >= 3 && t[0] == 'b' &&
		...
		return base64.StdEncoding.DecodeString(t[2 : len(t)-1]) //nolint:wrapcheck
	}
...
func (iq *IrondbQuery) ToCaql() (string, error) {
	if len(iq.Segments) == 0 {
		return "", fmt.Errorf("ParseBasic must be called to populate Segments before ToCaql") //nolint:goerr113
	} else if len(iq.Segments) > 1 {
		return "", fmt.Errorf("ToCaql currently supports only metric name queries") //nolint:goerr113
	}
