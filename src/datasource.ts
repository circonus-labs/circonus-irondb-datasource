...
export default class IrondbDatasource extends DataSourceApi<IrondbQueryInterface, IrondbOptions> {
    ...
    query(options: DataQueryRequest<IrondbQueryInterface>): Promise<DataQueryResponse> {
        if (!_.isUndefined(this.queryRange) && !_.isEqual(options.range, this.queryRange)) {
            log(
                () =>
                    'query() time range changed ' +
                    JSON.stringify(this.queryRange) +
                    ' -> ' +
    ...
    metricFindQuery(query: IronDBVariableQuery | string, options: any) {
        ...
        if (!_.isUndefined(range)) {
            ...
            if (!_.isUndefined(this.queryRange) && !_.isEqual(range, this.queryRange)) {
                log(
                    () =>
                        'query() time range changed ' + JSON.stringify(this.queryRange) + ' -> ' + JSON.stringify(range)
                );
                if (this.useCaching) {
                    log(() => 'query() clearing cache');
                    const requestCache = this.datasourceRequest as unknown as Memoized<(options: any) => any>;
        ...
        function removeResultWarning() {
            ...
            if (header) {
                Array.from(header.parentElement.parentElement.getElementsByClassName('result-warning')).forEach(
                    function (el) {
                        el.remove();
                    }
                );
    ...
    irondbRequest(optionsAll, followLimit = true) {
        ...
        if (irondbOptions['std']['names'].length) {
            const paneltype =
                irondbOptions['std']['names'][0]['leaf_data']['target']['format'] ||
                irondbOptions['std']['names'][0]['leaf_data']['format'] ||
                'ts';
            for (let i = 0; i < irondbOptions['std']['names'].length; i++) {
                ...
                stream['name'] =
                    irondbOptions['std']['names'][i]['leaf_data']['metric_name'] ||
                    irondbOptions['std']['names'][i]['leaf_name'];
        ...
        if (irondbOptions['caql']['names'].length) {
            for (let i = 0; i < irondbOptions['caql']['names'].length; i++) {
                ...
                const interval =
                    minPeriodDirective &&
                    calculatedInterval === Math.round(IrondbDatasource.MIN_DURATION_MS_CAQL / 1000)
    ...
    static checkRollupAligned(rollupMs) {
        ...
        if (
            rollupMs > IrondbDatasource.ROLLUP_ALIGN_MS_1DAY &&
            rollupMs % IrondbDatasource.ROLLUP_ALIGN_MS_1DAY !== 0
        ) {
            throw new Error('Unaligned rollup period requested');
        }
    }
    ...
    buildFetchStream(target, result, i, scopedVars) {
        ...
        // get the metric type
        let types = (('heatmap' === target.format ? 'histogram' : result[i]['type']) || 'numeric').split(',');
        let this_type;
        while (!this_type) {
            this_type = types.pop();
        }
    ...
    buildAlertQueryAsync(cleanOptions, target, start, end) {
        ...
        cleanOptions['alert']['labels'].push(
            this.templateSrv.replace(target['metriclabel'], cleanOptions['scopedVars'])
        );
